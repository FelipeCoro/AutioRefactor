...
class StoryOptionsAdapter<T>(
    ...
    private val onStoryOptionClickListener: ((StoryOption, T) -> Unit)?
) :
    ...
    inner class OptionViewHolder(
        ...
        fun render(model: StoryOption) {
            binding.root.setOnClickListener {
                onStoryOptionClickListener?.invoke(model, story)
            }
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int) =
        OptionViewHolder(
            ItemListPopupWindowBinding.inflate(LayoutInflater.from(parent.context), parent, false)
        )
