...
class SubscribeActivity :
                ...
                ?.let { packages ->
                    with(
                        ...
                        for (p in packages) {
                            when (p.product.sku) {
                                SINGLE_TRIP_PRODUCT -> {
                                    cvSingleTrip.setOnClickListener {
                                        makePurchase(
                                            p
                                        )
                                    }
                                ...
                                ADVENTURER_TRIP_PRODUCT -> {
                                    cvAdventurer.setOnClickListener {
                                        makePurchase(
                                            p
                                        )
                                    }
    ...
    private fun updateSubscriptionUI(
        ...
        val isSubscriptionActive =
            customerInfo?.entitlements?.get(
                REVENUE_CAT_ENTITLEMENT
            )?.isActive == true

        with(
            binding.tvSignInOrRestorePurchase
        ) {
            if (!isUserLogged) {
                // Sign in or restore purchase
                text =
                    resources.getText(
                        R.string.sign_in_or_restore_purchase
                    )
                makeLinks(
                    "Sign In" to View.OnClickListener {
                        goToLoginActivity()
                    },
