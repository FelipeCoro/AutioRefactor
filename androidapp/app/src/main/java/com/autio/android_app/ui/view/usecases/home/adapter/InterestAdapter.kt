...
class InterestAdapter(private val interestList: List<InterestModel>): RecyclerView.Adapter<InterestViewHolder>() {
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): InterestViewHolder {
        val layoutInflater = LayoutInflater.from(parent.context)
        return InterestViewHolder(layoutInflater.inflate(R.layout.item_interest,parent,false))
    }

    override fun onBindViewHolder(holder: InterestViewHolder, position: Int) {
        val item = interestList[position]
        holder.render(item)
    }
