...
class SignUpActivity : AppCompatActivity() {
    ...
    private fun createUser() {
       if (Utils.checkEmptyField(binding.tvName) ||
           Utils.checkEmptyField(binding.tvEmail) ||
           Utils.checkEmptyField(binding.tvPassword)){
           Utils.pleaseFillText(this)
       }else{
           val name = binding.tvName.text.toString()
           val password = binding.tvPassword.text.toString()
           val email = binding.tvEmail.text.toString()
           val createAccountDto = CreateAccountDto(email,email,password,password,name)
           apiService.createAccount(createAccountDto){
               Log.i("CREATE ACCOUNT:", "---------Im here----------")
               if (it != null){
                   saveUserInfo(it)
    ...
    private fun saveUserInfo(loginResponse: LoginResponse) {
        prefRepository.setIsUserGuest(false)
        prefRepository.setUserId(loginResponse.id!!)
        prefRepository.setUserApiToken(loginResponse.apiToken!!)
        prefRepository.setUserName(loginResponse.name!!)
        prefRepository.setUserEmail(loginResponse.email!!)
    }
}