...
class PackageValidator(
    ...
    private val callerChecked =
        mutableMapOf<String, Pair<Int, Boolean>>()
    ...
    fun isKnownCaller(
        ...
        val (checkedUid, checkResult) = callerChecked[callingPackage]
            ?: Pair(
                0,
                false
            )
        ...
        val callerPackageInfo =
            buildCallerInfo(
                ...
                ?: throw IllegalStateException(
                    "Caller wasn't found in the system?"
                )
        ...
        val isPackageInAllowList =
            certificateAllowList[callingPackage]?.signatures?.first {
    ...
    private fun buildCertificateAllowList(
        parser: XmlResourceParser
    ): Map<String, KnownCallerInfo> {

        val certificateAllowList =
            LinkedHashMap<String, KnownCallerInfo>()
        try {
            var eventType =
                parser.next()
            while (eventType != XmlResourceParser.END_DOCUMENT) {
                if (eventType == XmlResourceParser.START_TAG) {
                    val callerInfo =
                        when (parser.name) {
                            "signing_certificate" -> parseV1Tag(
                                parser
                            )
                            "signature" -> parseV2Tag(
                                parser
                            )
                            else -> null
                        }
                    ...
                    callerInfo?.let { info ->
                        val packageName =
                            info.packageName
                        val existingCallerInfo =
                            certificateAllowList[packageName]
    ...
    private fun parseV2Tag(
        ...
        val callerSignatures =
            mutableSetOf<KnownSignature>()
        var eventType =
            parser.next()
    ...
    private fun getSystemSignature(): String =
        getPackageInfo(
            ...
            ?: throw IllegalStateException(
                "Platform signature not found"
            )

    /**
     * Creates a SHA-256 signature given a Base64 encoded certificate.
     */
    private fun getSignatureSha256(
        certificate: String
    ): String {
        return getSignatureSha256(
            Base64.decode(
                certificate,
                Base64.DEFAULT
            )
    ...
    private fun getSignatureSha256(
        ...
        try {
            ...
            Log.e(
                TAG,
                "No such algorithm: $noSuchAlgorithmException"
            )
            throw RuntimeException(
                "Could not find SHA256 hash algorithm",
    ...
    companion object {
        private val TAG =
            PackageValidator::class.simpleName
        private const val ANDROID_PLATFORM =
            "android"
