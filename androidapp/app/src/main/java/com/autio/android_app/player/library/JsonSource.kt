...
internal class JsonSource(
    ...
    private suspend fun updateCatalog(): List<MediaMetadataCompat>? {
        return withContext(
            ...
            val mediaMetadataCompats =
                storyCat.stories.map { story ->
                    val jsonImageUri =
                        Uri.parse(
                            story.imageUrl
                        )
    ...
    private suspend fun downloadJson(): JsonCatalog {
        ...
        return Gson().fromJson(
            jsonObject.toString(),
            JsonCatalog::class.java
        )
    }

    companion object {
        private val TAG =
            JsonSource::class.simpleName
        const val ORIGINAL_ARTWORK_URI_KEY =
            "com.autio.android_app.JSON_ARTWORK_URI"
    }
}
